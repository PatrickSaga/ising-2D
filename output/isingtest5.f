     ...
     * 			  		 +A(i+1,j)+A(i,j-1)+A(i,j+1))
			...
			MC_loop2: do istep = 0, nstep
				...
				newspin = -A(m,n)
				dU=-2*newspin*(A(m-1,n)+A(m+1,n)+A(m,n-1)
     *       	                                             +A(m,n+1))
				if (dU < 0.d0) then
				  A(m,n) = newspin
     ...
     * 			  		+A(i+1,j)+ A(i,j-1)+A(i,j+1))
		...
		chi = beta*(mag2_avg/nvalues - (mag_avg/nvalues)**2)

		err_chi = beta*sqrt(mag4_avg/nvalues - 
     *        (mag2_avg/nvalues)**2)/sqrt(float(nvalues))

		sh = (beta**2)*(energy2_avg/nvalues-(energy_avg/nvalues)**2)

		err_sh = (beta**2)*sqrt(energy4_avg/nvalues - 
     *        (energy2_avg/nvalues)**2)/sqrt(float(nvalues))

		! write the output files
		write(21,*) T,',',mag_avg/nvalues,',',chi,',',err_chi
		write(22,*) T,',',energy_avg/nvalues,',',sh,',',err_sh
	enddo temp_loop
