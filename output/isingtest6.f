	...
	temp_loop: do iscan = 1, nscan
		...
		if (T .le. 2.275d0 .and. T .ge. 2.260d0) then		! Wolff near Tc
			MC_loop1: do istep = 0, 60000
				...
				if (istep > 10000) then
				  if (mod(istep,10) == 0) then  
					...
					if(istep == 60000-10) then
					 do d=1, L-1
					  do i = 2, L+1
					   do j = 2, L+1
					    if(i+d .ge. L+2) then
						k = d-L
						corr(abs(k))=corr(abs(k))+A(i,j)*A(i+k,j)
					    else
						corr(d)=corr(d)+A(i,j)*A(i+d,j)
					    endif
					    if(j+d .ge. L+2) then
						k = d-L
						corr(abs(k))=corr(abs(k))+A(i,j)*A(i,j+k)
					    else
						corr(d)=corr(d)+A(i,j)*A(i,j+d)
					    endif	
					   enddo
					  enddo
					 enddo
